...
# Takes [optionally] a minimum significance level (alpha), a maximum alpha and
# a number of steps to go between them, and [mandatorily] a data file name,
# alphabet file name, and runs CSSR multiple times, saving results by re-naming
# files, then calculates the BIC score for each model, looping over alpha and
# over history lengths.  Renames the selected model to indicate that it was
# selected, moves losers to a new directory for safe-keeping.

# Significance levels are stepped through from large to small, in steps of
# even logarithmic size, i.e., each step is a constant multiple of the last.
# The user sets the number of steps.

# Maximum history length set by the Marton-Shields formula, with the log of
# the alphabet size as a conservative over-estimate of the entropy rate.

# Optionally, prints out tables giving # states, likelihood, BIC and AIC vs.
# history length, and a verbal message on the winning length and its BIC.
...
# by a larger margin than does AIC, and so should be more reliable.  Option
# to select by AIC (or other IC?) may be added later.
...
my $usage = "--verbose [--alpha min max steps] <alphabet file> <data file>";
if ((@ARGV==3) || (@ARGV==7)) {
