...
/**
  ...
  */
class EvenProcess (val repetitions:Int) extends TestMachine {
  val numStates = 2
  val generator = new Random()
  val seed = generator.nextDouble()
  val emitOne:Tuple2[Double,Double] = (0.5, 1.0)
  val zeroTrans:Tuple2[Double,Double] = (0, 0) // But we should never see a zero in this state
  val oneTrans:Tuple2[Double,Double] = (1, 0)

  val stateCounts:ArrayBuffer[Double] = ArrayBuffer()
  val observedSequence:ArrayBuffer[Char] = ArrayBuffer()
  ...
  scaffoldFiles("evenProcess", alphabet, (pw) => {
    ...
    for (i <- 1 to repetitions) {
      ...
      val (observation, nextState) = makeAMove(currentState)
      observedSequence += observation
      pw.print(observation)
      currentState = nextState
      stateSequence += alphabet(currentState)
    }
  })
